...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims):
        ...
        # second conv
        self.conv2 = nn.Conv2d(8, 16, 3, stride=1, padding='same')
        ...
        # fourth conv
        self.conv4 = nn.Conv2d(32, 64, 3, stride=1, padding='same')
        self.max4 = nn.MaxPool2d(2, stride=2)
        
        # linear layers
        self.linear1 = nn.Linear(2*2*64, 32)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(vae.state_dict(), 'models/restart_vae_addSpots_r9.pt')
...
# plot predictions
# vae.load_state_dict(torch.load('models/restart_vae_addSpots_r9.pt', map_location = torch.device('cpu')))
# vae.eval()
