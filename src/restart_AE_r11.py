...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims):
        ...
        self.linear1_z = nn.Linear(16, latent_dims)
            ...
            for i in range(0, img.shape[1]-patch_size, img_shift):
                for j in range(i, img.shape[2]-patch_size, img_shift):
                    ...
                    if len(full_img_patches) == bs:
                        ...
                        # AE loss
                        loss = ((full_img_patches - full_img_patches_hat)**2).sum()
...
#         torch.save(vae.state_dict(), 'models/restart_AE_addSpots_r11.pt')
...
# plot predictions
vae.load_state_dict(torch.load('models/restart_AE_addSpots_r11.pt', map_location = torch.device('cpu')))
