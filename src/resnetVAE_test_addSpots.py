...
class VariationalAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        z, dist_z = self.encoder(x)
        # print(x.shape)
        out = self.decoder(dist_z)
        # print(x[0], x[0].shape)
...
vae.load_state_dict(torch.load('models/resnetVAE_addSpots_runFULL_2.pt', map_location = torch.device('cpu')))
vae.eval()
...
np.savez_compressed('gen_files/embeds_resnetVAE_BG_run2__O1.npz', embeds_vae)
print(patches_csv)
