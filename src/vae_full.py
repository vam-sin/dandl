...
def plot_ae_outputs(vae_model, n=1, patch_size=32):
    ...
    for x, _ in valid_loader:
        ...
        # print(full_img_patches.shape)
        # full_img_patches = np.expand_dims(patch_img, axis=0)
        ...
        print(full_img_patches.shape)
        full_img_patches_hat = vae(full_img_patches)
        print(full_img_patches_hat.shape)
        # plt.show()
        for s in range(n):
            # print(full_img_patches_hat[s].detach().numpy().shape)
            ...
            # print(orig_img, pred_img)
            # print(np.mean(orig_img))
            # print(z_embed[s])
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(vae.state_dict(), 'models/vae_bg_remap_runFULL_2.pt')
...
vae.load_state_dict(torch.load('models/vae_bg_remap_runFULL_1.pt', map_location = torch.device('cpu')))
vae.eval()
