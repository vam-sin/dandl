...
## Training function
def train_epoch(vae, device, dataloader, optimizer, patch_size = 16):
    vae.train()
    train_loss = 0.0
    img_num = 1
    num_samples = 0
...
## Testing function
def test_epoch(vae, device, dataloader, patch_size = 16):
    vae.eval()
    val_loss = 0.0
    num_samples_val = 0
    with torch.no_grad():
...
## plotting function
## plotting function
def plot_ae_outputs(vae_model, n=1, patch_size=16):
...
#         torch.save(vae.state_dict(), 'models/restart_vae_addSpots_r5.pt')
...
# plot predictions
vae.load_state_dict(torch.load('models/restart_vae_addSpots_r5.pt', map_location = torch.device('cpu')))
vae.eval()
